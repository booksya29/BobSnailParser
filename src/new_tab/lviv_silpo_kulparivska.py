...
async def select_addr(page:Page):
    ...
    print(addr_name)
    if 'Кульпарківська' in addr_name:
        return True
    ...
    await page.locator('button[autotestid="\'search-suggestions__item\'"]', has_text = 'Львів').click(timeout=3000,click_count=2)
    await asyncio.sleep(1)
    await page.locator('div[data-autotestid="search-branch"]').click(timeout=3000, click_count=2)
    await asyncio.sleep(1)
    await page.locator('button[class*="search-suggestions__item ft-w-full"]', has_text='Кульпарківська').click(timeout=3000, click_count=2)
