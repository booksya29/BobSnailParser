...
async def check_addr(page:Page) -> bool:
    try:
        ...
        print(addr_name)
        if "Степана Бандери" in addr_name:
            return True
        else:
...
async def set_addr(page:Page) -> bool:
    try:
        await page.locator('span[class*="AddressButton"]').first.click()
        await page.locator('li[data-marker="Pickup"]').first.click()
        await page.locator('div[class="SelectStyled__single-value css-r447wv-singleValue"]').nth(1).click()
        await page.locator('div[id*="react-select-"]', has_text='Auchan Почайна DRIVE').click()
        await page.locator('span[class*="Button__text"]', has_text='покуп').click()
...
async def test():
    async with async_playwright() as pw:
        ...
        urls = ['https://auchan.zakaz.ua/uk/products/tsukerka-bob-sneil-60g-ukrayina--04820219345435/', 'https://auchan.zakaz.ua/uk/products/sous-priprava-komo-150g-ukrayina--04820285119954/', 'https://auchan.zakaz.ua/uk/products/kovbasa-nasha-riaba-apetitna-300g-ukrayina--04823116846111/']
        for i in urls:
